...
public class SchemeResourceIT {
  ...
  @Autowired private SchemeMapper schemeMapper;

  @Autowired private SchemeService schemeService;

  @Autowired private SchemeQueryService schemeQueryService;

  @Autowired private EntityManager em;

  @Autowired private MockMvc restSchemeMockMvc;
  ...
  public void getAllSchemes() throws Exception {
    ...
    restSchemeMockMvc
        .perform(get("/api/schemes?sort=id,desc"))
  ...
  public void getScheme() throws Exception {
    ...
    // Get the scheme
    restSchemeMockMvc
        .perform(get("/api/schemes/{id}", scheme.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
  ...
  public void getAllSchemesBySchemeDescriptionIsInShouldWork() throws Exception {
    ...
    // Get all the schemeList where schemeDescription in DEFAULT_SCHEME_DESCRIPTION or
    // UPDATED_SCHEME_DESCRIPTION
    defaultSchemeShouldBeFound(
        "schemeDescription.in=" + DEFAULT_SCHEME_DESCRIPTION + "," + UPDATED_SCHEME_DESCRIPTION);
  ...
  private void defaultSchemeShouldBeFound(String filter) throws Exception {
    restSchemeMockMvc
        .perform(get("/api/schemes?sort=id,desc&" + filter))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
        .andExpect(jsonPath("$.[*].id").value(hasItem(scheme.getId().intValue())))
        .andExpect(jsonPath("$.[*].schemeName").value(hasItem(DEFAULT_SCHEME_NAME)))
    ...
    restSchemeMockMvc
        .perform(get("/api/schemes/count?sort=id,desc&" + filter))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
  ...
  public void getNonExistingScheme() throws Exception {
    // Get the scheme
    restSchemeMockMvc
        .perform(get("/api/schemes/{id}", Long.MAX_VALUE))
  ...
  public void updateScheme() throws Exception {
    ...
    //        // Disconnect from session so that the updates on updatedScheme are not directly saved
    // in db
