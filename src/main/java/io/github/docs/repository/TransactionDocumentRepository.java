...
public interface TransactionDocumentRepository extends JpaRepository<TransactionDocument, Long>, JpaSpecificationExecutor<TransactionDocument> {
    ...
    @Query(value = "select distinct transactionDocument from TransactionDocument transactionDocument left join fetch transactionDocument.documentOwners",
    ...
    @Query("select distinct transactionDocument from TransactionDocument transactionDocument left join fetch transactionDocument.documentOwners")
    List<TransactionDocument> findAllWithEagerRelationships();

    @Query("select transactionDocument from TransactionDocument transactionDocument left join fetch transactionDocument.documentOwners where transactionDocument.id =:id")
